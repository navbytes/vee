...
> **Nothing here has been compiled or run.** The implementation was written in
> a Linux cloud session with no Swift toolchain, against a package that targets
> macOS 26 + AppKit. A ticked box below means the code and its tests are
> written and reviewed by eye, not that they build or pass. The first job on a
> Mac is the gate command above; expect to fix typos before judging any of it.

## 1. The placement value and its storage

- [x] 1.1 Add `Sources/VeePreferences/BarPlacement.swift`: the enum from
...
- [x] 1.2 `AppPreferences`: `defaultPlacement` derived from the existing
...
- [x] 1.3 Extend `clearAllState(id:)` to clear placement, and include
...
- [x] 1.4 Rename `compactMenuBarDidChangeNotification` to
...
- [x] 2.1 Add the `pluginID: String` init parameter (design.md §5) and pass it
  from `PluginCoordinator` alongside the existing `autosaveName`.
- [x] 2.2 Replace the `isCompact` flag with a resolved `BarPlacement`, and
...
- [x] 2.3 Add the no-bar-surface branch to `applyPresentation`, `applyMenu`,
  `applyTitleText` and `applyAlpha`: with both surfaces nil they store state
  and paint nothing.
  - As built: **no branches were needed** — all four already reach their
    surface through `if let … else if let …` or optional chaining. What did
    need a guard is `startCyclingIfNeeded` (no 3s timer for a title drawn
    nowhere), with `repaintCurrentSurface()` restarting it on the way back
...
- [x] 2.4 Guard the shared item's error roll-up so it only tracks folded rows,
...
- [ ] 3.1 Rename `CompactMenuBarController` → `VeeHomeItemController` (file,
  ...
  - **Deliberately not done here**, and best done first on a Mac: a cross-file
    rename is trivially safe with a compiler and error-prone without one. The
    behaviour merge (3.2/3.3) is complete under the old name
- [x] 3.2 Make the item permanent: the app-controls footer is installed once at
...
- [x] 3.3 `MainMenuController` stops creating an `NSStatusItem`; it remains the
...
- [x] 4.1 `PluginCoordinator` resolves the plugin's placement at construction
...
- [x] 4.2 Confirm `<vee.surface>widget` plugins are untouched: still no
  controller, still no menu-mode schedule, and placement is not offered for
  them in the UI.

## 5. User interface

- [x] 5.1 `GeneralSettingsView`: the Menu Bar section becomes a
...
- [x] 5.2 Plugin Manager rows: a per-plugin placement control (own / folded /
  ...
  - As built: the row's overflow menu gained Refresh, Open in Window, and a
    "Menu Bar" submenu; `PluginCoordinator.openWindow()` is the new seam.
    Edit-in-editor was left off the row — it is reachable via Reveal in Finder
    and was not worth a second new coordinator method unverified
  - Not visually checked: no toolchain, so the menu's layout, the checkmark
    rendering, and the SF Symbol choices are unverified by eye
...
- [ ] 7.1 `swift build && swift test && swiftlint lint --strict` — **the first
  thing to run**, since none of the above has been compiled. Known risk spots:
  the `switch` expressions in `PluginManagerView`'s `defaultPlacementLabel` /
  `placementSymbol`, the `Substring == "folded"` comparison in
  `BarPlacement.init(encoded:)`, and the new tests' reliance on the existing
  main-queue drain pattern for placement notifications.
