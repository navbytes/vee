...
| `accessoryWidth` | `number \| "full"` | Inline accessory width in points, sizing whichever accessory (sparkline, progress bar, chart, slider) the item carries. `"full"` stretches it to the row's width, where supported. Supersedes `sparklineWidth`/`progressWidth`/`chart.w`. |
