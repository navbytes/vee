...
Read this if you are an LLM/agent generating a **Vee plugin**. Working on Vee's own
source instead? The house rules are in the repo-root `AGENTS.md` — surface parity,
`openspec/`, and the build gate — and `CONTRIBUTING.md` has style and the PR flow.
